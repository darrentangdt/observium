...
if (!is_device_mib($device, 'CISCO-IPSEC-FLOW-MONITOR-MIB'))
{
  ...
  $valid['ipsec_tunnels'] = array();

  // Cache DB entries
  $t_db = dbFetchRows('SELECT * FROM `ipsec_tunnels` WHERE `device_id` = ?', array($device['device_id']));
  foreach ($t_db as $t)
  {
  ...
  */
  ...
  $json_oids = array('cipSecTunIkeTunnelIndex', 'cipSecTunIkeTunnelAlive', 'cipSecTunLocalAddr', 'cipSecTunRemoteAddr',
  ...
  $table_rows = array();
  $table_headers = array('%WIndex%n', '%WLocal%n', '%WRemote%n', '%WName%n', '%WState%n', '%WIKE Alive%n', '%WHC%n');

  foreach ($ipsec_poll as $index => $tunnel)
  {
    foreach (array('cipSecTunLocalAddr', 'cipSecTunRemoteAddr') as $oid)
    {
      if (isset($tunnel[$oid])) { $tunnel[$oid] = hex2ip($tunnel[$oid]); }
    }
    ...
    else if (isset($ike_poll_index[$local_address][$peer_address]))
    {
      // FIX for cisco issue, not correct IKE index
      // https://bst.cloudapps.cisco.com/bugsearch/bug/CSCvb16714/
      $ike_exist = TRUE;
    ...
    if ($tunnel_insert)
    {
      ...
      if ($db_update)
      {
        $updated   = dbUpdate($db_update, 'ipsec_tunnels', '`tunnel_id` = ?', array($tunnel_db['tunnel_id']));
      }
    }
    ...
    if ($ike_exist)
    {
      ...
      rrdtool_update_ng($device, 'cipsec-tunnels', array(
